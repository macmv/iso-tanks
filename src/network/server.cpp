...
void Server::update_player_events(const PlayerProto& player, const PlayerEvents& events) {
  if (events.shoot().exists()) {
    world->add_projectile(player.id(), events.shoot().projectile());
  }
}
...
void Server::create_res(uint id, bool needsPositionSet, PlayerUpdateResponse* res) {
  res->clear_player();
  for (std::pair<uint, Player*> item : *world->players) {
    if (!(item.first == id && !needsPositionSet)) {
