...
ClientWorld::ClientWorld(Terrain* terrain, bool needs_debug, SceneManager* scene_manager, ParticleManager* particle_manager) {
...
void ClientWorld::draw_debug() {
  debug_render->update();
}

void ClientWorld::create_this_player(Controller* controller, Camera* camera) {
...
void ClientWorld::add_player(uint id) {
  rp3d::RigidBody* body = add_body(glm::translate(glm::mat4(1), glm::vec3(0, -970, 0)));
  body->addCollider(player_box_shape, rp3d::Transform::identity());
  body->updateMassPropertiesFromColliders();
...
bool ClientWorld::has_player(uint id) {
  return players->find(id) != players->end();
}

ControlledPlayer* ClientWorld::get_this_player() {
  return this_player;
}

bool ClientWorld::move_this_player(glm::mat4 transform) {
  this_player->set_transform(transform);
  return true;
}

bool ClientWorld::move_player(uint id, glm::mat4 transform) {
  Player* p = players->at(id);
  p->set_transform(transform);
  return true;
}

void ClientWorld::update_controls() {
...
void ClientWorld::force_physics_update() {
...
void ClientWorld::update() {
  ...
  while (accumulator > PHYSICS_STEP) {
    ...
    if (duration.count() > PHYSICS_STEP * 2 && accumulator > PHYSICS_STEP * 2) {
      world->update((double) accumulator / 1000000000);
      accumulator = 0;
    } else {
...
// client function
void ClientWorld::add_projectile(ProjectileProto proto) {
...
bool ClientWorld::has_projectile(uint id) {
  return projectiles->find(id) != projectiles->end();
}

rp3d::RigidBody* ClientWorld::add_body(glm::mat4 trans) {
  world_mutex.lock();
